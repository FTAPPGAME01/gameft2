...
io.on('connection', (socket) => {
    ...
    // Agregar este nuevo event handler
    socket.on('requestCurrentState', () => {
        console.log('Cliente solicitó estado actual:', socket.id);
        socket.emit('stateChanged', gameState);
    });
    
    socket.on('playerConnected', (username) => {
        console.log('Jugador conectado:', username);
        // Asegurarse de que el jugador tenga una entrada en el objeto score
        if (!gameState.score[username]) {
            gameState.score[username] = 60000;
        }
        
        // Asegurarse de que el jugador tenga una entrada en takenRowsByPlayer
        if (!gameState.takenRowsByPlayer[username]) {
            gameState.takenRowsByPlayer[username] = [];
        }
        
        // Notificar a todos los clientes sobre el jugador conectado
        io.emit('playerStatusUpdate', { 
            player: username, 
            status: 'online' 
        });
    });
     // Modificar el handler updateState existente
     socket.on('updateState', (updatedState) => {
        // Guardar información de jugadores actuales antes de actualizar
        const currentPlayers = Object.keys(gameState.score);
        const currentTakenRows = {...gameState.takenRowsByPlayer};
        
        // Actualizar el estado con los cambios recibidos
        gameState = updatedState;
        
        // Asegurarse de preservar información de jugadores que no están en el estado actualizado
        currentPlayers.forEach(player => {
            if (!gameState.score[player]) {
                gameState.score[player] = 60000;
            }
            
            if (!gameState.takenRowsByPlayer[player] && currentTakenRows[player]) {
                gameState.takenRowsByPlayer[player] = currentTakenRows[player];
            }
        });
        
        // Verificar si es necesario reiniciar el juego
        if (gameState.takenCount >= 16) {
            resetGame();
        } else {
...
// Modificar la función resetGame para preservar información de jugadores
const resetGame = () => {
    // Guardar información importante antes de reiniciar
    const currentScores = {...gameState.score};
    const players = Object.keys(currentScores);
    
    // Reiniciar el tablero
    initializeBoard();
    
    // Restaurar información importante
    gameState.score = currentScores;
    gameState.currentPlayer = 'Ruperto'; // O el jugador que debe ir primero
    gameState.timeLeft = 10;
    
    // Reiniciar el objeto takenRowsByPlayer para todos los jugadores
    gameState.takenRowsByPlayer = {};
    players.forEach(player => {
        gameState.takenRowsByPlayer[player] = [];
    });
